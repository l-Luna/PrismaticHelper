...
public static class StylegroundsPanelRenderer{
	...
	public static void BeforeRenderStylegroundPanels(bool fg, Scene level, BackdropRenderer renderer){
		List<IGrouping<string, AbstractPanel>> toUpdate = level.Tracker
			.GetEntities<StylegroundsPanel>()
			.Cast<AbstractPanel>()
			.Where(it => it.Foreground == fg)
	...
	public static void Load(){
		...
		On.Celeste.BackdropRenderer.BeforeRender += OnBackdropRendererBeforeRender;
	}
	...
	public static void OnBackdropRendererBeforeRender(On.Celeste.BackdropRenderer.orig_BeforeRender orig, BackdropRenderer self, Scene scene){
		orig(self, scene);
		if(scene is Level level)
			if(self == level.Foreground)
				BeforeRenderStylegroundPanels(true, level, self);
			else if(self == level.Background)
				BeforeRenderStylegroundPanels(false, level, self);
	}
