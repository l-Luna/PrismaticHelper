...
internal class MultiLockedDoor : Solid{
	...
	private void CollectKeys(Player player){
		...
		int keysCollected = player.Leader.Followers.Count(k => k.Entity is Key{ StartedUsing: false });
	...
	private void TryOpen(Player player){
		...
		if(!Scene.CollideCheck<Solid>(player.Center, Center)){
			...
			foreach(var key in player.Leader.Followers.Where(k => k.Entity is Key{ StartedUsing: false }).Select(k => (Key)k.Entity)){
